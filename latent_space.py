...
def search(sess, tensor, save_tensor=False):
    ...
    zp = tf.Variable(np.random.normal(
        size=(BATCH_SIZE, Z_NOISE_DIM)), dtype=tf.float32)
...
def interpolate(sess, tensor_a, tensor_b, steps=50):
...
def interpolate_images(sess, image_a, image_b, steps=50):
    tensor_a, _ = search_image(sess, image_a)
    tensor_b, _ = search_image(sess, image_b)
    interpolate(sess, tensor_a, tensor_b, steps)
