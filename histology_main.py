...
def run():
    ...
    parser.add_argument("--generate", nargs='?', const=1, type=int, help="num of examples to generate")
    ...
    with tf.Session() as sess:
        ...
        if args.generate:
            output.generate_samples(sess,z_batch_tensor, input_z, last_global_step, args.generate, save_tensor=True)   
            ...
            train(sess, saver,z_batch_tensor, increment_global_step, dataset_len, iterator, input_real, input_z, d_loss, g_loss, d_train_opt, g_train_opt)
    ...
    while True:
        try:
            ...
            if steps % SAVE_MODEL_EVERY == 0:
                saver.save(sess, BASE_PATH + CHKPTS_PATH + "model", global_step=steps)
            ...
            if steps % SAVE_EXAMPLE_EVERY == 0:
                ...
                output.save_output(sess, z_batch_tensor, input_z, steps, num_samples=3)
            gc.collect()
